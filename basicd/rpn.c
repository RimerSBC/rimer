...
_bas_err_e rpn_eval(uint8_t op)
{
   ...
   switch (op) // type resolution
   {
   case OPERATOR_NOT:
      break;
   case OPERATOR_BWNOT:
      if (value[0].type & VAR_TYPE_FLOAT) return BasicError = BASIC_ERR_TYPE_MISMATCH;
      break;
   case OPERATOR_BWAND:
   case OPERATOR_BWOR:
   case OPERATOR_BWXOR:
      value[1] = *rpn_pull_queue();
      if ((value[0].type & VAR_TYPE_FLOAT) || (value[1].type & VAR_TYPE_FLOAT)) return BasicError = BASIC_ERR_TYPE_MISMATCH;
      break;
   case OPERATOR_BWSL:
   case OPERATOR_BWSR:
      value[1] = *rpn_pull_queue();
   ...
   // Bitwise operations
   case OPERATOR_BWNOT:
      value[0].var.i = ~value[0].var.i;
      break;
   case OPERATOR_BWAND:
      value[0].var.i &= value[1].var.i;
      break;
   case OPERATOR_BWOR:
      value[0].var.i |= value[1].var.i;
      break;
   ...
   case OPERATOR_BWSR:
      if (value[1].type & VAR_TYPE_FLOAT) return BasicError = BASIC_ERR_TYPE_MISMATCH;
      if (value[0].type & VAR_TYPE_FLOAT) value[0].var.i = (int32_t)value[0].var.f;
      value[0].var.i = value[1].var.i >> value[0].var.i;
      break;
   default:
      switch (op)
